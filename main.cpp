...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	DirectX::ScratchImage LoadTexture(const std::string & filePath)
	{
		...
		HRESULT hr = DirectX::LoadFrowWICFile(filePathW.c_str(), DirectX::WIC_FLAGS_FORCE_SRGB, nullptr, image);
		assert(SUCCEEDED(hr));
	...
	void UpLoadTextureData(ID3D12Resouce* texture, const DirectX::ScratchImage& mipImages)
	{
		//Meta情報を取得
		const DirectX::TexMetadata& metadata = mipImages.GetMetadata();
		//全MipMapについて
		for (size_t mipLevel = 0; mipLevel < metadata.mipdata.mipLevels; ++mipLevel) {
